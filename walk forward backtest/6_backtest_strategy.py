"""
Step 8: Walk-Forward Backtest with Decay Weighting
===================================================

SIMPLIFIED VERSION - Single horizon with decay weighting only.

Testing showed:
- Multi-month horizons (2-12): NO significant improvement
- Daily confirmations (5d, 10d, 15d): NO significant improvement
- Decay weighting (54-month half-life): SIGNIFICANT improvement for N=4,5

This script implements the optimal strategy:
- Single 1-month holding horizon
- Exponential decay weighting (54-month half-life)
- Greedy forward feature selection

Usage:
    python 8_multi_horizon_with_decay.py

Output:
    data/backtest_results/decay_backtest_summary.csv
"""
...
# Decay weighting - statistically significant for N=4,5
DECAY_HALF_LIFE_MONTHS = 54  # Center of 48-60 significant range

# Single horizon (multi-horizon showed no benefit)
HOLDING_MONTHS = 1
...
# ============================================================
# FAST FEATURE SELECTION WITH DECAY WEIGHTING
# ============================================================

def fast_greedy_search_with_decay(data, n_satellites, train_mask, test_idx):
    """
    Ultra-fast greedy ensemble search using Numba-parallelized functions.
    NOW WITH EXPONENTIAL DECAY WEIGHTING.
    """
    ...
    # Step 1: Evaluate ALL features with decay weights in parallel (Numba)
...
# ============================================================
# SELECT TOP-N ETFs (Single Horizon - No Consensus Needed)
# ============================================================
...
def select_etfs(data, test_idx, n_satellites, feature_indices, rankings):
    ...
    Args:
        ...
        test_idx: Index of test date
        n_satellites: Number of ETFs to select
        feature_indices: List of feature indices to use
        rankings: Modified rankings matrix (with inverted features)

    Returns:
        List of selected ISIN indices, or None if not enough valid
    """
...
# ============================================================
# WALK-FORWARD BACKTEST WITH DECAY WEIGHTING
# ============================================================

def walk_forward_backtest(data, n_satellites, show_progress=True):
    """
    Run walk-forward backtest with decay-weighted feature selection.

    Single 1-month horizon only (multi-horizon showed no benefit).
    """
    dates = data['dates']
    isins = data['isins']
    alpha_matrix = data['alpha_matrix']
    ...
    for test_idx in iterator:
        ...
        if months_since_reopt >= REOPTIMIZATION_FREQUENCY:
            ...
            # Use decay-weighted feature selection
            selected, train_perf, modified_rankings = fast_greedy_search_with_decay(
...
def main():
    """Run walk-forward backtest with decay weighting."""
    print("=" * 60)
    print("WALK-FORWARD BACKTEST WITH DECAY WEIGHTING")
    print("=" * 60)
    print(f"\nDecay half-life: {DECAY_HALF_LIFE_MONTHS} months")
    ...
    # Load data (single 1-month horizon only)
    data = load_data()
    ...
    for test_num, n in enumerate(N_SATELLITES_TO_TEST, 1):
        ...
        if len(results_df) > 0:
            ...
            if stats:
                ...
                output_file = OUTPUT_DIR / f'decay_backtest_N{n}.csv'
    ...
    if len(all_stats) > 0:
        ...
        # Save summary
        summary_file = OUTPUT_DIR / 'decay_backtest_summary.csv'
